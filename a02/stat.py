...
def _(mo):
    ...
    vol = mo.ui.slider(label='Объем выборки:', start=50, stop=10_000, value=1500, step=20, full_width=True, debounce=True)
    bins = mo.ui.slider(label='Число разрядов гистограммы:', start=5, stop=200, step=5, value=50, full_width=True, debounce=True)
    distrs = mo.ui.radio(label="Распределение:",
           options={'Нормальное':1, 'Равномерное':2, 
                    'Экспоненциальное':3, 'Бимодальное':4}, 
            value='Нормальное')
    width = mo.ui.slider(label='Ширина рисунка:', start=2, stop=12, step=1,  value=3, full_width=True, debounce=True)
    height = mo.ui.slider(label='Высота рисунка:', start=4, stop=12, step=1,  value=5, full_width=True, debounce=True)   
...
def _(bins, distrs, get_data, height, mo, title, vol, width):
    ...
    def ui():
        ...
        lh = mo.vstack([mo.md('### Данные:'), vol, bins, distrs,
                       ...
                       mo.md('### Храктеристики распределения:'), stats])
